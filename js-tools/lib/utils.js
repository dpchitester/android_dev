...
function makelinks() {
	...
	ns.forEach(function(n, i, a) {
		s += ':' + n + crlf;
		s += 'rem %~d0\\Programs\\Utils\\Shortcut.exe /F:' + n + '.lnk /A:C /T:\"%%' + n + '%%\" /D:\"Opens explorer in ' + n + '\"' + crlf;
		// s +='rem echo \\plaunch explorer.exe %%' + n + '%% > explore_' + n + '.bat' + crlf;
		s += crlf;
	});
	require('fs').writeFileSync(__dirname.substring(0, 2) + '\\.bat\\makelinks.bat', s, {
		encoding: 'ascii'
	});
}
...
// var str = '%LOCALAPPDATA%\\Google\\Chrome\\Application';
	var replaced = s.replace(/%([^%]+)%/gi, function(_,n) { return process.env[n]; });
	return replaced;	
}
