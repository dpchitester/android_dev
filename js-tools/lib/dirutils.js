		...
		function(ra) {
			...
			d2fl.forEach(
				function(d2fld) {
					p3 = p3.then(
						function() {
							return diskutils.delFile(d2fld.fp).then(
								function() {
			...
			return p3.then(
				function() {
					return diskutils.rmDir(d2).then(
						function() {
							dmEE1.emit('ddel', d2);
						}
					);
...
function delDirs(d1, d2) {
	...
	return Promise.all([p1, p2]).then(
		function(rva) {
			var d1dl = rva[0];
			var d2dl = rva[1];
			var toDelete = d2dl.filter( // not in source
				function(cdd) {
...
var copycrit = [
	...
	function(s, t) { // 2
		...
		// 	return Promise.reject();
		// else {
		if (t.mtime.getTime() < s.mtime.getTime())
			return Promise.reject();
		else
			return Promise.resolve();
		// }
				...
				function(d1fld) {
					...
					p4 = p4.catch(
						function() {
							...
							return diskutils.copyFile(sfp, dfp).then(
								function() {
...
function dirCopy(d1, d2, exs) {
	fcs.clr();
	return diskutils.dirExists(d2).then(
		function(de) {
			if (!de) mkDirs(d2);
			return copyFiles(d1, d2, exs, 1 | 2).then(
				function() {
					fcs.fcstats.dcpy++;
...
function touchBats() {
	...
	if (setExe()) {
		return diskutils.filesList(utils.expES('%FLASH0%\\'), ['.bat']).then(
			function(fl) {
...
function dirTreeWalk(pth, fn, norecurse, exc_dirs) {
	...
	function not_exc(pth3) {
		return exc_dirs.filter(
			function(e) {
				var b1 = pth3.length >= e.length;
				var b2 = pth3.substring(0, e.length)
					.toLowerCase()
	...
	if (!exc_dirs || not_exc(pth2)) {
		if (!norecurse) {
			p3 = p3.then(
				function() {
					return diskutils.dirsList(pth2).then(
						function(dirs) {
							var p4 = Promise.resolve();
							dirs.forEach(
								function(dir) {
...
function mkDirs(dir, mode) { // from jake
