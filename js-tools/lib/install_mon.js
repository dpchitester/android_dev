...
var watcher = ch.watch(
	wdirs[0], {
		persistent: false,
		ignored: [
			/clog_flash_KINGSTON.json/,
			/.lock/
		],
		cwd: null
	}
...
function f2() {
	...
	Object.keys(tdirs).forEach(
		function(k) {
			...
			p1 = p1.then(
				function() {
					return dirutils.dirSync(sfp, dfp).then(
						function() {
							utils.writedata('\r\n');
						}
					);
