...
function establish() {
	if (typeof process.env.FLASH0 == 'undefined' || typeof process.env.FLASH1 == 'undefined') return;
...
function dirCopy(d1, d2) {
	...
	fcs.clr();
	addDMListeners();
	return dirutils.dirCopy(d1, d2, []).then(
		function() {
			removeDMListeners();
			fcs.print();
			json.writeConfig(clog, cfn);
...
function flashSync1(d1, d2) {
	...
	fcs.clr();
	addDMListeners();
	return dirutils.dirMirror(d1, d2).then(
		function() {
			removeDMListeners();
			clearTimeout(cbf);
			clearInterval(psf);
...
function dsPro() {
	...
	var cmd2 = 'call \\plaunch DirSyncPro.exe ' + dscfn2.replace(process.env.FLASH0, "%FLASH0%") + '\r\n';

	var kcdsc_xml = fs.readFileSync(dscfn1, 'ascii');
	if (!kcdsc_xml) return Promise.reject();
	return new Promise(
		function(resolve, reject) {
			new xml2js.Parser().parseString(kcdsc_xml,
				function(err, result) {
					if (err) reject(err);
					else
						resolve(result);
...
function sublimeText() {
	...
	var cmd2 = 'call \\plaunch subl.exe --project ' + spfn1.replace(process.env.FLASH0,'%FLASH0%') + '\r\n';
